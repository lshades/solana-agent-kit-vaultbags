...
| `VAULTBAGS_GET_PAYOUT_INTEGRITY` | `getPayoutIntegrity()` | Every payout ever recorded, looked up on Solana and counted: accepted, rejected, or no longer in the answering node's history. Recomputed on read. `allLanded` is true only when every one was found AND accepted. Serves nothing rather than a figure it could not verify in full. |
| `VAULTBAGS_GET_LOCK_TIER` | `getLockTier({ days?, wallet? })` | The tier a lock term reaches, or the one a wallet holds. Both optional and independent. Derived from the term signed for, never stored, and it grants nothing: the boost multiplier does not read it, and the same term reaches the same tier at any balance. |
| `VAULTBAGS_GET_TREASURY_HISTORY` | `getTreasuryHistory({ days?, points? })` | The treasury's value and per-asset balances over time. Samples span the WHOLE window, not its newest end, so a 90-day request describes 90 days. Descriptive history, never a projection. |
| `VAULTBAGS_GET_HOLDER_DISTRIBUTION` | `getHolderDistribution()` | Distinct holding wallets (not token accounts; protocol wallets and pools excluded), the change over a day and a week, how many are locking, and how concentrated supply is. Aggregates only, no addresses. Concentration is null rather than approximated when its scan could not complete. |
| `VAULTBAGS_GET_RECENT_ACTIVITY` | `getRecentActivity()` | Recent trading for the last day and week: trades, buys, sells and distinct buyers, sellers and traders. `truncated` marks a window whose counts are a floor rather than a total. |
